...
# from med_seg_diff_pytorch.wo_attn import Unet, MedSegDiff
# from med_seg_diff_pytorch.diff_contr import Unet, MedSegDiff
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.save_every == 0:
            ...
            pred_noise, t2d_img = diffusion.sample(tse_img)
            pred_noise = pred_noise.cpu().detach().numpy()
            t2d_img = t2d_img.cpu().detach().numpy()

            visualize_list = []
            for i in range(t2d_img.shape[0]):
                tmp = np.concatenate([to_8bit(t2d_img[i, ::]), to_8bit(dess_img[i:i+1, ::].cpu().detach().numpy()), to_8bit(tse_img[i:i+1, ::].cpu().detach().numpy())], 1)
