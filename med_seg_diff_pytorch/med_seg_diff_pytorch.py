...
class Unet(nn.Module):
    ...
    def forward(
        ...
        c = self.cond_init_conv(cond) # (b, 64, 256, 256)
        t = self.time_mlp(time) # (b, 256)
        h = []
        # x : noise feature, t: time feature, c: conditional feature
        for (block1, block2, attn, downsample), (cond_block1, cond_block2, cond_attn, cond_downsample), conditioner in zip(self.downs, self.cond_downs, self.conditioners):
            # 看起來有4層
            #          block 1 / cond block 1    attn / cond attn         after fft       downsample / cond downsample
            # 第一層     (b, 64, 256, 256)       (b, 64, 256, 256)     (b, 64, 256, 256)       (b, 64, 128, 128)
            # 第二層     (b, 64, 128, 128)       (b, 64, 128, 128)     (b, 64, 128, 128)       (b, 64, 64, 64)
            # 第三層     (b, 64, 64, 64)         (b, 64, 64, 64)       (b, 64, 64, 64)         (b, 64, 32, 32)
            # 第四層     (b, 64, 32, 32)         (b, 64, 32, 32)       (b, 64, 32, 32)         (b, 64, 32, 32)
            #
            # mid block 1 -> 吃 (x, t) -> (b, 512, 32, 32) -> 出 x
            # cond mid block 1 -> 吃 (c, t) -> (b, 512, 32, 32) -> 出 c
            # x = x + c
            # mid transformer 吃 x -> (b, 512, 32, 32)
            # mid block 2 -> 吃 (x, t) -> (b, 512, 32, 32)
            ####################################
               ########## DECODER ###########
            ####################################
            # x 跟 r 相接   (b, 64, 256, 256) cat (b, 63, 256, 256)
            # final res block : 吃相接後的x, t -> 出(5, 64, 256, 256)
            # final conv : 吃(5, 64, 256, 256) -> 出(5, 1, 256, 256)
            x = block1(x, t)
            c = cond_block1(c, t)
...
class MedSegDiff(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t, c, x_self_cond = None, clip_denoised = True): # x: (2, 5, 256, 256) c: (2, 256, 256)
    ...
    def p_sample_loop(self, shape, cond):
        ...
        # A tmp sol for pain used
        img = torch.randn(shape, device = device)
        # img = torch.randn((batch, shape[1], 384, 384), device=device)
        x_start = None
    ...
    def sample(self, cond_img):
        ...
        cond_img = cond_img.to(self.device) #(2, 256, 256)
    ...
    def p_losses(self, tmp_not_used_mask, tse_img, x_start, t, noise = None):
        b, c, h, w = x_start.shape
        x_start = x_start.float() # dess
        tse_img = tse_img.float() # tse
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        if self.self_condition and random() < 0.5:
            with torch.no_grad():
                # predicting x_0
                x_self_cond = self.model_predictions(x, t, tse_img).pred_x_start
                x_self_cond.detach_()

        # predict and take gradient step
        # noise_img, time, dess_img
        model_out = self.model(x, t, tse_img, x_self_cond)
        ...
        model_out_x_start = self.predict_start_from_noise(x, t, model_out)

        if self.i % 30 == 0:
            visualize_list = []
            for i in range(x.shape[0]):
                tmp = np.concatenate([to_8bit(x[i, ::].cpu().detach().numpy()), to_8bit(tse_img[i, ::].cpu().detach().numpy()),
                                      to_8bit(x_start[i, ::].cpu().detach().numpy()), to_8bit(model_out[i, ::].cpu().detach().numpy()),
                                      to_8bit(model_out_x_start[i, ::].cpu().detach().numpy())], 1)
                visualize_list.append(tmp)
            tmp = np.concatenate(visualize_list, 2)
            cv2.imwrite(f'add_l2_after_0.png', np.transpose(tmp, (1, 2, 0)))
        self.i += 1
        # Added L1 compare x_start and pain
        # if self.i > 100000:
        #     global_are_loss = F.mse_loss(model_out_x_start, normalize_to_neg_one_to_one(tse_img))
        # else:
        #     global_are_loss = 0
        return F.mse_loss(model_out, target) # + global_are_loss

    def forward(self, dess_img, cond_mask, tse_img, *args, **kwargs):
        if dess_img.ndim == 3:
            dess_img = rearrange(dess_img, 'b h w -> b 1 h w')

        if tse_img.ndim == 3:
            tse_img = rearrange(tse_img, 'b h w -> b 1 h w')

        # if cond_mask.ndim == 3:
        #     cond_mask = rearrange(cond_mask, 'b h w -> b 1 h w')

        device = self.device
        dess_img, tse_img = dess_img.to(device), tse_img.to(device) #, cond_mask.to(device)
        b, c, h, w, device, img_size, img_channels, mask_channels = *dess_img.shape, dess_img.device, self.image_size, self.input_img_channels, self.mask_channels

        assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        # assert cond_mask.shape[1] == mask_channels, f'your input medical must have {mask_channels} channels'
        assert dess_img.shape[1] == img_channels, f'the segmented image must have {img_channels} channels'

        times = torch.randint(0, self.num_timesteps, (b,), device = device).long()

        dess_img = normalize_to_neg_one_to_one(dess_img)
        return self.p_losses(cond_mask, tse_img, dess_img, times, *args, **kwargs)
