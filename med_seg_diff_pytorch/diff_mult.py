...
class MedSegDiff(nn.Module):
    ...
    def model_predictions(self, x, t, c, x_self_cond = None, clip_x_start = True):
    ...
    def p_losses(self, tmp_not_used_mask, tse_img, dess_img, t, noise = None):
        x_start = dess_img / (tse_img + 0.0001)
        maybe_clip = partial(torch.clamp, min=-1, max=1.)
        x_start = maybe_clip(x_start)
        x_start = (x_start - 0.5) / 0.5 # norm to (-1, 1)
        ...
        reconstruct_p2np = ((model_out_x_start + 1) / 2) * tse_img
        visualize_list = []
        for i in range(x.shape[0]):
            tmp = np.concatenate([to_8bit(x[i, ::].cpu().detach().numpy()),
                                  to_8bit(dess_img[i, ::].cpu().detach().numpy()),
                                  to_8bit(tse_img[i, ::].cpu().detach().numpy()),
                                  to_8bit(x_start[i, ::].cpu().detach().numpy()),
                                  to_8bit(model_out[i, ::].cpu().detach().numpy()),
                                  to_8bit(model_out_x_start[i, ::].cpu().detach().numpy()),
                                  to_8bit(reconstruct_p2np[i, ::].cpu().detach().numpy()),
                                  ], 1)
            visualize_list.append(tmp)
        tmp = np.concatenate(visualize_list, 2)
        cv2.imwrite(f'mulitply_0.png', np.transpose(tmp, (1, 2, 0)))
